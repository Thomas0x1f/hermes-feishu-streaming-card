...
async def emit_from_hermes_locals_async(
    ...
    try:
        ...
        if is_terminal_emit:
            _hfc_warn(
                f"{event_name} emit failed: {_hfc_exception_summary(exc)} "
                "(native fallback will deliver)"
            )
        return False
...
def _exact_terminal_payload(
    ...
    *,
    ...
    # Base 已原生投递媒体：卡片不再上传，也不该把本地路径带进 payload/日志。
...
def _hfc_handle_interaction_select_action(
    ...
    try:
        ...
        _hfc_warn(f"{action} forward failed: {_hfc_exception_summary(exc)}")
        return _hfc_empty_feishu_callback_response(adapter)
