...
- **卡片内交互**：approval / clarify choices 渲染为按钮；Hermes 原生 `multi_select=true` 的 clarify 自动渲染为 `multi_select_static + submit` 真多选卡并返回选项数组；`/new`、`/reset`、`/undo`、`/model` 等独立命令使用原生 interactive card。V4 的 `/model` 与 Hermes CLI 使用同一 Provider/模型列表，按 Provider → Model 两级选择，不再把全部模型挤进一个下拉框。
...
| [v3.8.6](docs/release-notes-v3.8.6.md) / [v3.8.5](docs/release-notes-v3.8.5.md) | Docker/source-stripped Hermes 缺 `VERSION` 时用 Gateway anchors 兜底，兼容 Hermes v0.18.0；更早历史修复见 release notes |
