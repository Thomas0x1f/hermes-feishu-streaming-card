...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `HFC_VERSION` | `latest` | 指定安装版本，例如 `v3.8.1`、`v3.6.6` 或 `main` |
...
| `HFC_VERSION` | `latest`（脚本）/ `v3.8.1`（Compose 示例） | 指定安装 tag 或分支 |
...
从 V3.2.x/V3.3.0/V3.4.x/V3.5.x/V3.6.x/V3.7.x/V3.8.0 升级到 V3.8.1 向后兼容，**单 Profile 配置无需任何修改**。如果 Hermes 使用自己的 venv，升级后请重新跑 `setup` 或 `install`，让插件同时进入 Hermes runtime Python 并刷新 hook。V3.8.1 还新增 Gateway-side delta 合并和 `/hfc` 只读诊断命令，建议升级后执行一次 `doctor --explain`，并在飞书里发送 `/hfc status` 验证 sidecar 状态。
