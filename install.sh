...
load_env_file() {
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    case "$line" in
      FEISHU_APP_ID=*|FEISHU_APP_SECRET=*|FEISHU_CONNECTION_MODE=*|FEISHU_HOME_CHANNEL=*|HERMES_FEISHU_CARD_HOST=*|HERMES_FEISHU_CARD_PORT=*|HERMES_FEISHU_CARD_PROFILE_ID=*|HERMES_FEISHU_CARD_EVENT_URL=*|HERMES_FEISHU_CARD_NOTICE_UNCERTAIN_WARNING_ENABLED=*|HFC_CONFIG=*|HFC_VERSION=*|HFC_NO_REPAIR=*)
        local key="${line%%=*}"
        local value="${line#*=}"
