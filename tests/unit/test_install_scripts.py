...
""",
    ...
    assert (
        ...
        "git+https://github.com/baileyh8/hermes-feishu-streaming-card.git@main" in log
    )
...
def test_install_docker_sh_explicit_local_source_allows_credential_free_smoke(tmp_path):
    ...
    assert result.returncode == 0, result.stderr + result.stdout
    log = (tmp_path / "python.log").read_text(encoding="utf-8")
    assert f"-m pip install --upgrade --force-reinstall --no-deps {ROOT}" in log
...
def test_install_docker_sh_uses_container_defaults_and_hermes_venv(tmp_path):
    ...
    assert (
        "-m pip install --upgrade --force-reinstall --no-deps "
        "git+https://github.com/baileyh8/hermes-feishu-streaming-card.git@v3.7.0" in log
    )
...
def test_install_docker_sh_uses_latest_without_pin(tmp_path):
    ...
    assert f"-m pip install --upgrade --force-reinstall --no-deps {spec}" in log
